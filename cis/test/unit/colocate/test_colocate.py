...
class TestCollocate(unittest.TestCase):
    def test_get_kernel(self):
        from cis.collocation.col_implementations import moments, li, nn_gridded, nn_pressure, mean
        from cis.collocation.col import get_kernel

        assert isinstance(get_kernel('moments'), moments)
        assert isinstance(get_kernel('nn_gridded'), nn_gridded)
        assert isinstance(get_kernel(li()), li)
