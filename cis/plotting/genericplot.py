...
class GenericPlot(APlot):
    ...
    def __init__(self, packed_data_items, *mplargs, **mplkwargs):
        ...
        self.x, self.data = self.xaxis.data, packed_data_items.data

        self.mplkwargs['label'] = self.label or packed_data_items.long_name

        self.plot()

        self.ax.set_xlabel(self.guess_axis_label(packed_data_items, self.xaxis))
        self.ax.set_ylabel(self.guess_axis_label(packed_data_items))
...
class Generic2DPlot(APlot):
    ...
    def __init__(self, packed_data_items, ax=None, logv=False, vmin=None, vmax=None, vstep=None,
        ...
        self.unpack_data_items(packed_data_items, self.xaxis, self.yaxis, x_wrap_start)
        ...
        self.ax.set_xlabel(self.guess_axis_label(packed_data_items, self.xaxis))
        self.ax.set_ylabel(self.guess_axis_label(packed_data_items, self.yaxis))

        self.ax.set_title(packed_data_items.name())

    def unpack_data_items(self, data_object, x, y, x_wrap_start=None):
        """
