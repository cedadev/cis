...
def can_set_line_colour_to_valid_colour():   
    ...
    plot([cube], out_filename = out_filename, **{"color" : valid_colour})
...
def can_set_line_style_to_valid_style():   
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, **{"linestyle" : valid_line_style})   
    
@nottest # Jenkins can't plot to file or screen
def can_set_colour_map_to_valid_colour():   
    cube = make_cube(valid_2d_filename)
    plot([cube], out_filename = out_filename, **{"cmap" : valid_colour_map})
    
@nottest # Jenkins can't plot to file or screen
@raises(ValueError)
def should_raise_valueerror_with_invalid_colour_map():   
    cube = make_cube(valid_2d_filename)
    plot([cube], out_filename = out_filename, **{"cmap" : invalid_colour_map})

@nottest # Jenkins can't plot to file or screen    
def can_set_font_size_to_valid_size():
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, **{"fontsize" : valid_font_size})

@nottest # Jenkins can't plot to file or screen    
def can_set_plot_width_alone():
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, **{"width" : valid_width})
    
@nottest # Jenkins can't plot to file or screen    
def can_set_plot_height_alone():
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, **{"height" : valid_height})
    
@nottest # Jenkins can't plot to file or screen    
def can_set_plot_width_and_height():
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, **{"height" : valid_height, "width" : valid_width})
    
@nottest # Jenkins can't plot to file or screen    
def can_set_ymin_alone():
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, valrange = {"ymin" : valid_ymin})
    
@nottest # Jenkins can't plot to file or screen    
def can_set_ymax_alone():
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, valrange = {"ymax" : valid_ymax})
    
@nottest # Jenkins can't plot to file or screen    
def can_set_ymin_and_ymax():
    cube = make_cube(valid_1d_filename)
    plot([cube], out_filename = out_filename, valrange = {"ymin" : valid_ymin, "ymax" : valid_ymax})
