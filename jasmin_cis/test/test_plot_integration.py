...
def should_do_scatter_plot_of_valid_2d_file():
    ...
    arguments = ['plot', valid_1d_vairable+':'+valid_2d_filename+':product=NetCDFGriddedByVariableName', '--output', valid_2d_filename+'.png']
    main_arguments = parse_args(arguments)
    plot_cmd(main_arguments)
...
def should_do_scatter_plot_of_valid_2d_file_with_x_as_lat_y_as_long():
    # Actual file name: xglnwa.pm.k8dec-k9nov.col.tm.nc
    # Trac issue #153
    arguments = ['plot', valid_1d_vairable+':'+valid_2d_filename+':product=NetCDFGriddedByVariableName', '--xaxis', 'latitude', '--yaxis',
                 'longitude', '--output', valid_2d_filename+'.png']
    main_arguments = parse_args(arguments)
    plot_cmd(main_arguments)
...
def should_do_heatmap_plot_of_valid_1d_file():
    ...
    # Trac issue #145
    arguments = ['plot', valid_1d_vairable+':'+valid_1d_filename+':product=NetCDFGriddedByVariableName', '--output',
...
def should_do_line_plot_of_valid_zonal_time_mean_cmip5_file():
    ...
    # Trac issue #130
    arguments = ['plot',
            valid_zonal_time_mean_CMIP5_variable+':'+valid_zonal_time_mean_CMIP5_filename+':product=NetCDFGriddedByVariableName',
